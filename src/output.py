...
def return_put(params, predef_param, best_cost, containers, meta, I_rep, t_lth):
    ...
    data = np.concatenate((start+[state, N, I_rep, t_lth, best_cost], params[:-3], predef_param, params[-3:], containers))
...
def param_df_out_single(data, meta):
    ...
    df = pd.DataFrame(data, columns=start+['state','population','I_reported','series_length','best_cost','beta1','beta2','delta','ha','gamma_H','gamma_U','t_thresh','kappa','p','gamma_asym','gamma_sym','ksi','mi_H','mi_U','omega_H','omega_U','initial_I_asym','initial_I_sym','initial_E','S','I_asym','I_sym','E','H','U','R','D','Nw'])
