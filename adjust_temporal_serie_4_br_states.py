...
def observed_data():
    ...
    ids = set(raw.state) # getting unique cities

    cities = dict()

    for id in ids:
        city_lines = raw[raw.state == id]
...
def city_metadata():
    last_date = get_last_date('.', 'estado_sumario_')
    df = pd.read_csv(f'estado_sumario_{last_date}.csv', index_col=0)
    ...
    for id in df.index:
        city_row = df.loc[id]
        metadata[id] = (
            city_row.populacao, # [0] -> N population scalar
            id                  # [1] -> State name
        )
...
def save_time_series(params, predef_param, containers, meta, extra_days):
    ...
    outpath = f'{out_folder()}/{state}/time_series_extraDays={extra_days}.csv'
...
def param_df_out(data, observations, cities):
    ...
    df = pd.DataFrame(data, columns=['state','population','series_length','best_cost','beta1','beta2','delta','h','ksi','t_thresh','kappa','p','gamma_asym','gamma_sym','gamma_H','gamma_U','mi_H','mi_U','omega','initial_I_asym','initial_I_sym','initial_E','S','I_asym','I_sym','E','H','U','R','D','Nw'])
...
def hard_work(q, thr, cities, city_meta, observed):
    ...
    for city in cities:
        c+=1
        print(f'{thr}:[{c}/{tot}] Started {city}, {city_meta[city][1]}...\n')
